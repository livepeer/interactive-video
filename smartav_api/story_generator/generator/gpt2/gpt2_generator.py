...
class GPT2Generator:
    ...
    def generate_multiple_raws(self, prompt):
        """ This method will generate the multiple raw texts for the input prompt
        """
        context_tokens = self.enc.encode(prompt)
        generated = 0
        generated_text_list = []
    ...
    def generate_multiple(self, prompt, options=None, seed=1):
        """ Generate multiple texts
        """
        prompt = self.prompt_replace(prompt)

        generated_text_list = self.generate_multiple_raws(prompt)
        ...
        for text in generated_text_list:
            ...
            if len(result) == 0:
                text_again = self.generate(prompt)
                replaced_text_list.append(text_again)
            
            else:
                replaced_text_list.append(result)
