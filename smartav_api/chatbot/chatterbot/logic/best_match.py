from chatbot.chatterbot.logic import LogicAdapter
from chatbot.chatterbot import filters
...
class BestMatch(LogicAdapter):
    ...
    def process(self, input_statement, candidates_size, additional_response_selection_parameters=None):
        ...
        max_threshold = self.maximum_similarity_threshold
        for result in search_results:
            # Find the closet match
            if result.confidence >= max_threshold:
                closest_match = result
                max_threshold = result.confidence
        ...
        if closest_match.search_in_response_to != '':
