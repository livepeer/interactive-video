...
class UnconstrainedStoryManager(StoryManager):
    def act(self, action_choice, result_type='single'):
        if result_type == 'multiple':
            results = self.generate_multiple_results(action_choice)
            self.story.add_to_story(action_choice, None)
            return results
        else:
    ...
    def generate_multiple_results(self, action):
        block_list = self.generator.generate_multiple(self.story_context() + action)
        return block_list
